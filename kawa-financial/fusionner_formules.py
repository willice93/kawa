...
def fusionner_fichiers(id_base):
    ...
    if not os.path.exists(path_exp) or not os.path.exists(path_ex):
        print(f"[!] Exemple manquant pour : {id_base}")
        return
    ...
    yaml_struct = {
        "id": base,
        "titre": explication.strip().split('\n')[0],
        "source": "Brochure IR 2024",
        "regle": {
            "description": explication.strip(),
            "formule": formule if formule else "Non spécifiée"
        },
        "cas_pratique": exemple_txt.strip()
    }
    if formule and exemple_calc and exemple_calc["abattement"] > 0:
        yaml_struct["calcul"] = {
            "formule": formule,
        ...
        if formule:
            yaml_struct["regle"]["formule"] = "Détectée, mais sans données exploitables"
    ...
    if formule:
        print(f"[✓] YAML avec formule : {output_path}")
    else:
        print(f"[✓] YAML sans formule : {output_path}")
